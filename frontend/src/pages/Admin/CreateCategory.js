...
const CreateCategory = () => {
  ...
  const handleUpdate = async(e)=>{
    e.preventDefault();
    try{
        const {data} = await axios.put(`/category/update-category/${selected._id}`, {name:updateName});
        if(data.success){
        ...
        toast.error("Something went wrong in the Updation");
    }
  }
  ...
  const handleDelete = async(pid)=>{
    try{
        ...
        toast.error("Something went wrong in the Deletion");
    }
  }
  return (
            ...
            <div className="row">
                    ...
                    <h1>Manage Category</h1>
                        ...
                        <Modal onCancel={()=>setVisible(false)} footer={null} visible={visible}>
                            <CategoryForm value={updateName} setValue={setUpdateName} handleSubmit={handleUpdate}></CategoryForm>
                        </Modal>
                    </div>
                </div>
            </div>
